...
<%--
	...
	JSP 구성
		...
		- 스크립트요소 : HTML 안에서 자바코드를 사용할 수 있도록 만든 요소들
			...
			3) 표현식 (<%= 내용 %>) : 변수 값이나 메소드의 결과값을 출력할 때 사용
			
			** 주의사항 : 스크립트요소들은 겹쳐서 사용하면 안된다.
	
		1. 지시어 (디렉티브) : <%@ page %>, <%@ taglib %>, <%@ include %>
			가. page 지시어 : 보통 해당 문서의 맨 앞에 나온다.
							현재 JSP 페이지에 대한 정보를 제공
							톰켓이 해당 page지시어를 보고, 해당 페이지를 어껗게 해석하고 실행할지 판단
							이클리스는 자동으로 만들어준다.(개발자가 필요하지 않으면 고치지 않는다.)
			  	page 지시어 속성들
			  	 ...
			  	 - autoFlush : 출력 버퍼가 다 차지 않아도 바로바로 출력 시켜줌 (기본 : true)
			  	 - errorPage : 해당 페이지가 오류를 발생했을 때 처리하는 오류 페이지 지정
			  	 - isErrorPage : 해당 페이지가 오류를 처리하는 페이지이면 true, 아니면 false(기본 : false)
