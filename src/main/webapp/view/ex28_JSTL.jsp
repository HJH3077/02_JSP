...
<%--
	...
	2. 다운 받은 라이브러리 WEB-INF안에 lib 폴더를 만들어서 넣어주자
	3. 지시어 중 <%@ taglib %>에 선언하고 사용한다. ss
 ...
 <%--
  ...
  <%-- 	if문은 else가 없는 불편함이 있어서 choose 문을 사용한다. 
  		(choose문은 switch문과 비슷하다.(if~else문 대신으로 많이 사용))  
  	  	 <c:choose>
  	 	  	<c:when test="조건식(관계연산, 비교연산 => EL방식으로 사용)">참일때 실행</c:when>
  	 	  	<c:when test="조건식(관계연산, 비교연산 => EL방식으로 사용)">참일때 실행</c:when>
  	 	  	<c:when test="조건식(관계연산, 비교연산 => EL방식으로 사용)">참일때 실행</c:when>
  	 	  	<c:otherwise>위 조건식이 모두 거짓일 때, 즉 나머지.</c:otherwise>
  	 	 </c:choose>
   --%>
   	<%-- avg의 점수가 80이상이면 합격 아니면 불합격 --%>
  	 <c:choose>
  	 	<c:when test="${avg>=80 }"><li>합격</li></c:when>
  	 	<c:when test="${avg<80 }"><li>불합격</li></c:when>
  	 </c:choose>
  	 <hr>
  	 
  	 <%-- avg의 점수가 90이상이면 A학점, 80이상이면 B학점, 70이상이면 C학점, 나머지는 F학점 --%>
  	 <%-- ** 정수는 정수로 비교, 실수는 실수로 비교해야 한다. --%>
  	 ...
  	 <%-- 
  	 	반복문
  	 	 ...
  	 	 <c:forEach var="사용변수" item="${배열, 리스트(컬렉션)}">
  	 	 	반복 내용 (EL 활용)
  	 	 </c:forEach>
  	 --%>
	 ...
	 <h3> 21-30까지 출력하기</h3>
	 <%-- vatStatus ; count, index, (first, last => boolean) --%>
	...
	<%request.setCharacterEncoding("utf-8"); %>
	 ...
	 <c:redirect url="ex08.jsp">
	 	이렇게 끝태그를 /로 바꿔서 처리 가능한 것을 xml이라고 한다.
	 	(끝태그 처리해야하는 것은 html)
