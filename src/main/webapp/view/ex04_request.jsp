...
<body>
<!-- 
	request : 웹 프로그래밍에서 가장 많이 사용되는 클래스 중 하나이다.
			  클라이언트가 요청한 정보를 담고 있는 클래스 (요청정보 => 파라미터 값도 가지고있다.)
			  
		** request의 생명주기는 response가 이루어지면 끝난다.
		
		request가 가지고 있는 정보들
		 - 클라이언트의 정보 및 서버 정보를 얻을 수 있는 메소드 제공
		 - from 정보와 요청 파라미터의 정보를 제공
		 - 요청 헤드의 정보 제공
		 
		 1) 클라이언트의 정보 및 서버 정보를 얻을 수 있는 메소드
		 	- getRemoteAddr()			- getContentlength()
		 	- getCharacterEncoding()	- get ContentType()
			- getProtocol()				- get ContentPath()
		 ...
		 3) 요청 헤드 정보			
		 	...
		 	- getDateheader(String name): 헤더 값을 시간 값으로 구하기
		 ...
		 4) 다른 주요 메소드
		 	...
		 	- request 속성을 통해서 저장된 데이터 호출 : request.setAttribute("이름"); 		 	
 -->
