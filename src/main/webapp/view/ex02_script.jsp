...
<body>
	<%--
		...
		1) 선언부 (<%! 내용 %>) : 변수 선언이나 메소드 선언을 할 때 사용
		2) 스크립트릿 (<% 내용 %>) : 자바코드를 코딩할 때 사용
		3) 표현식 (<%= 내용 %>) : 변수 값이나 메소드의 결과값을 출력할 때 사용(나중에 EL이나 JSTL로 변경한다.)
 ...
 --%>
	...
	<%
	for (int i = 0; i < 11; i++) {
		// System.out.println(); : 이건 콘솔에 출력하는 것이므로 의미가 없다.
		// 웹 페이지, 브라우저에 출력 
		// 서블릿 처럼 out 객체가 존재해서 사용 가능(태그 사용 가능)
		// &nbsp : 띄어쓰기
		out.println(i + "&nbsp;&nbsp;");
	}
	%>
	...
	%>
	<p><%= year %>년 <%= month %>월 <%= day %>일 당신의 운세는 <%= lucky %>% 입니다.</p>
