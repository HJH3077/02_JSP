...
<!-- 
	response(응답) : request와 반대 기능을 수행한다.
		...
		2. 자바스크립트를 통해서 이동
		 ...
		 1번과 2번은 웹 페이지에서 이벤트 처리를 해야하지만 다른 페이지로 이동
 ...
 -->
 	...
 	<%-- a 링크 --%>
 	<a href="ex08.jsp">ex08.jsp페이지로 이동(파라미터 없음)</a><br>
 	<a href="ex08.jsp?name=일지매&age=18">ex08.jsp페이지로 이동(파라미터 있음)</a><br>
 	<hr>
 	
 	<%-- javascript --%>
 	<button onclick="send_go1()">ex08.jsp페이지로 이동(파라미터 없음)</button>
 	<button onclick="send_go2()">ex08.jsp페이지로 이동(파라미터 있음)</button>
 	<hr>
 	
 	<%-- 리다이렉트 : 최종 주소가 보임 --%>
	<%-- <% response.sendRedirect("ex08.jsp"); %> --%>
	<%-- <% response.sendRedirect("ex08.jsp?name=둘리&age=22"); %> --%>
	<!-- 리다이렉트는 이벤트 없이 실행하므로 실행하면 바로 ex08.jsp로 넘어감 -->
